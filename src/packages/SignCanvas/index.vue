    ...
    methods: {
        ...
        canvasClear () {
            ...
            if (this.config.isShowBorder && !this.config.isSign) {
                //画里面的框
                for (var i = 1; i < 8; i++) {
                  this.context.moveTo(i * this.canvas.width / 8, 0);
                  this.context.lineTo(i * this.canvas.width / 8, this.canvas.height)
                  this.context.stroke();
                  this.context.moveTo(0, i * this.canvas.height / 8);
                  this.context.lineTo(this.canvas.width, i * this.canvas.height / 8);
                  this.context.stroke();
                }
        ...
        dealImage()  {
            ...
            //var ocx=c.getContext('2d');
            var img = new Image();

            var canvas_tmp = document.createElement('canvas');
            var dealWidth = 1600;    //目标尺寸
            var dealHeight = 800;
            canvas_tmp.width = dealWidth;
            canvas_tmp.height = dealHeight;
            ...
            dataURL = canvas_tmp.toDataURL('image/png')

            canvas_tmp = null;
            img=null;

            this.saveFile(dataURL, `${Date.parse(new Date())}.${this.config.imgType}`);
